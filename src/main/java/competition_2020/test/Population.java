...
public class Population {
    ...
    public void access() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (chrom.isTrue()) {
                chrom.setFitness(500 / (alpha * chrom.getDis() + beta * (1 - chrom.getAveLoadRate()) + gamma * chrom.getRoutes().size()));
                if (bestChrom == null) {
                    bestChrom = chrom;
                } else {
                    if (chrom.getFitness() > bestChrom.getFitness()) {
                        //  System.out.println(chrom.getFitness()+"  "+bestChrom.getFitness());
                        bestChrom = chrom;
                    }
                }
            } else {
        ...
        while (newChroms.size() < size - 1) {
            ...
            if (r <= Pc) {
                if (!equal(parent[0], parent[1])) {
                    Chrom[] childs = orderedCrossOver(parent);
                    newChroms.addAll(Arrays.asList(childs));
                } else {
                    continue;
                }
            } else {
                Chrom p1 = new Chrom(parent[0]);
                Chrom p2 = new Chrom(parent[1]);
                newChroms.add(p1);
                newChroms.add(p2);
            }
        }
     ...
     * @return  子代两个个体
     */
    public Chrom[] orderedCrossOver(Chrom[] parent) {
        ...
        childs[0] = new Chrom(child1);
        childs[1] = new Chrom(child2);
        return childs;
    }
    ...
    public void swap(Population population1) {
        ...
        s1.add(bestChrom);
        s2.add(population1.getBestChrom());
        ...
        this.chroms.addAll(s1);
        population1.getChroms().addAll(s2);
    }
...
//            if (curWeight + demand[num] > carTypes[maxCapacityIndex].getCapacity()||(dis[last][num]>10&&last!=0)) {
        ...
        public void mutate(){
            Random rd = new Random();
            int index1 = rd.nextInt(array.size());
            int index2 = rd.nextInt(array.size());
